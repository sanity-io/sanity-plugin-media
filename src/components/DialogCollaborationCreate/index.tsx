...
import {DialogCollaborationCreateProps} from '../../types'
import {collaborationActions} from '../../modules/collaborations'

type Props = {
  children: ReactNode
  dialog: DialogCollaborationCreateProps
}

const DialogCollaborationCreate = (props: Props) => {
  const {
    children,
    dialog: {id}
  } = props
  ...
  const onSubmit: SubmitHandler<TagFormData> = formData => {
    ...
    dispatch(collaborationActions.createRequest({name: sanitizedFormData.name}))
    dispatch(dialogActions.clear())
  }
...
export default DialogCollaborationCreate
